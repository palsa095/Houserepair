    ...
    <div class="mb-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        <div class="flex gap-2">
          ...
          {{-- Reset Filter Button --}}
          <a href="{{ route('surveys.index') }}" class="flex items-center rounded-lg bg-indigo-600 p-2 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
      ...
      <div class="flex gap-2">
        {{-- Print Button --}}
        <button onclick="printSurveyTable()" class="flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300">
    ...
    @if ($surveys->isEmpty())
      ...
      <div class="rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-8 text-center dark:border-gray-600 dark:bg-gray-800">
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Belum ada data survey yang tersimpan.
        </p>
        <div class="mt-6">
          <x-primary-button x-on:click="$dispatch('open-modal', 'createSurvey')">
            <i class="fa-solid fa-plus mr-2"></i>Tambah Survey Pertama
          </x-primary-button>
        </div>
      </div>
    @else
      <div class="overflow-x-auto">
        <table class="w-full border-collapse text-left text-sm" id="surveyTable">
          ...
          <tbody>
            @foreach ($surveys as $i => $survey)
              <tr class="border-b dark:border-gray-600">
                ...
                <td class="max-w-xs truncate p-2">{{ $survey->hasil_survey }}</td>
                <td class="p-3">
                  @if ($survey->dokumentasi)
                    @php
                      $images = json_decode($survey->dokumentasi);
                      $count = count($images);
                    @endphp
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                              @foreach ($images as $image)
                                <div class="overflow-hidden rounded border dark:border-gray-600">
                                  <img src="{{ asset('storage/' . $image) }}" alt="Dokumentasi survey" class="h-48 w-full object-cover">
                ...
                <td class="flex gap-2 p-2">
                  <button class="text-yellow-500 hover:text-yellow-700" @click="$dispatch('open-modal', 'editSurvey{{ $survey->id }}')">
                    <i class="fa-solid fa-pen"></i>
                  </button>
                  <button class="text-red-500 hover:text-red-700" @click="$dispatch('open-modal', 'deleteSurvey{{ $survey->id }}')">
    ...
    function printSurveyTable() {
      ...
      @foreach ($surveys as $i => $survey)
        ...
        [
          ...
          '{{ $survey->nama }}',
          '{{ $survey->hasil_survey }}',
          '{{ $survey->dokumentasi ? count(json_decode($survey->dokumentasi)) . ' foto' : 'Tidak ada' }}',
        ].forEach(text => {
